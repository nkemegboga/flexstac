...
parser.add_argument('--run_eval', default=False, type=str2bool, help='Flag to run eval')
...
def test_net(save_folder, net, cuda, dataset, transform, top_k,
    ...
    output_dir = get_output_dir(save_folder, "pkl")
...
if __name__ == '__main__':
    # load net
    net = build_ssd('test', ssd_dim, num_classes=num_classes)    # initialize SSD
    net.load_state_dict(torch.load(args.trained_model))
    #net.load_pretrained_npy(args.trained_model)
    net.eval()
    print('Finished loading model!')
    # load data
    dataset = VOCDetection(args.voc_root, [('2007', set_type)], BaseTransform(ssd_dim, dataset_mean), AnnotationTransform())
    if args.cuda:
        net = net.cuda()
    ...
    if args.run_eval:
        ...
        print("Will read model evaluation from {}".format(args.save_folder))
        output_dir = get_output_dir(args.save_folder, "pkl")
        det_file = os.path.join(output_dir, 'detections.pkl')
        with open(det_file, 'rb') as handle:
